...
CarbonIntensity = Annotated[
    ...
    Field(
        ...
        description=(
            ...
            "Simulated range: 150 (high solar penetration, 09\u201316h) "
            "to 800 (coal reliance, 18\u201306h)."
        ),
        examples=[210.5, 680.0],
    ),
]
...
class TelemetryReadingIn(BaseModel):
    ...
    # Optional on ingest — defaults to None so legacy clients and non-edge
    # payloads continue to work without modification.
    carbon_intensity_gco2_kwh: CarbonIntensity | None = Field(
        default=None,
        description=(
            "Grid carbon intensity in gCO\u2082/kWh [150–800]. "
            "Generated by the simulator based on time of day; "
            "NULL for legacy readings."
        ),
    )
...
class TelemetryReadingOut(BaseModel):
    ...
    # Sustainability field — populated at ingest time from the simulator / seed script
    carbon_intensity_gco2_kwh: float | None = Field(
        default=None,
        description=(
            "Grid carbon intensity in gCO\u2082/kWh [150–800]. "
            "None for readings ingested before the sustainability feature."
        ),
    )
